...
* **기본 비활성화 사유**: 웹사이트의 보안 헤더(`Content-Security-Policy`, `X-Frame-Options`)를 무조건적으로 제거하는 것은 사용자에게 보안 리스크(클릭재킹 등)를 유발할 수 있으며, 크롬 웹스토어의 보안 정책에 위배될 수 있기 때문입니다.
...
* **보안 경고**: 이 규칙을 활성화(접두사 제거)하면 미리보기 화면 내에서 대상 사이트의 보안 헤더가 강제로 제거됩니다. 사이트 호환성은 해결되지만, 신뢰할 수 없는 사이트에서는 **미리보기 샌드박스가 잠재적인 보안 취약점에 노출될 수 있으므로** 신중하게 사용하십시오.
