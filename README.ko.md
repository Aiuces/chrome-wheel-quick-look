...
* **비활성화 사유**: 일부 사이트는 보안 정책상 iframe 내부 로드를 차단하므로, 미리보기 팝업에서 "연결 거부(Connection Refused)" 오류가 발생할 수 있습니다.
* **사용 방법**: 특정 사이트가 정상적으로 로드되지 않는 경우, 해당 규칙에서 `disabled-` 접두사를 제거하여 프레임 제한을 우회할 수 있습니다.
* **보안 경고**: 이 규칙을 활성화(접두사 제거)하면 미리보기 컨텍스트 내에서 엄격한 보안 헤더(`Content-Security-Policy` 및 `X-Frame-Options`)가 강제로 제거됩니다. 사이트 호환성은 크게 향상되지만, 신뢰할 수 없는 사이트에서는 **미리보기 샌드박스가 잠재적인 보안 취약점에 노출될 수 있으므로** 주의하여 사용하십시오.
...
본 프로젝트는 엄격한 **개인적 비상업용 및 재배포 금지 라이선스(Custom Non-Commercial & Anti-Redistribution License)**에 따라 라이선스가 부여됩니다. 자세한 내용은 [LICENSE](LICENSE) 파일을 참조하십시오. 모든 상업적 이용 및 웹스토어 무단 재게시를 엄격히 금지합니다.
